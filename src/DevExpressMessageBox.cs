...
namespace Atomus.Control.MessageBox
{
    ...
    /// </summary>
    public class DevExpressMessageBox : IMessageBox
    {
        DialogResult IMessageBox.Show(string text, params string[] args)
        {
            return ((IMessageBox)this).Show(text, null, args);
        }
        DialogResult IMessageBox.Show(string text, string caption, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(text.Translate(args), caption.Translate());

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(messageResult.Message.Translate(args), caption.Translate());
        ...
        DialogResult IMessageBox.Show(string text, string caption, MessageBoxButtons messageBoxButtons, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(text.Translate(args), caption.Translate(), messageBoxButtons);

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(messageResult.Message.Translate(args), caption.Translate(), messageBoxButtons);
        ...
        DialogResult IMessageBox.Show(string text, string caption, MessageBoxButtons messageBoxButtons, MessageBoxIcon messageBoxIcon, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(text.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon);

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(messageResult.Message.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon);
        ...
        DialogResult IMessageBox.Show(string text, string caption, MessageBoxButtons messageBoxButtons, MessageBoxIcon messageBoxIcon, MessageBoxDefaultButton messageBoxDefaultButton, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(text.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon, messageBoxDefaultButton);

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(messageResult.Message.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon, messageBoxDefaultButton);
        ...
        {
            return XtraMessageBox.Show(exception.Message, exception.GetType().Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        DialogResult IMessageBox.Show(Exception exception)
        {
            if (exception.InnerException != null)
                return XtraMessageBox.Show(exception.InnerException.Message, exception.InnerException.GetType().Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
            else
                return XtraMessageBox.Show(exception.Message, exception.GetType().Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        DialogResult IMessageBox.Show(IWin32Window _Owner, string text, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(_Owner, text.Translate(args));

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(_Owner, messageResult.Message.Translate(args));
        ...
        DialogResult IMessageBox.Show(IWin32Window _Owner, string text, string caption, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(_Owner, text.Translate(args), caption.Translate());

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(_Owner, messageResult.Message.Translate(args), caption.Translate());
        ...
        DialogResult IMessageBox.Show(IWin32Window _Owner, string text, string caption, MessageBoxButtons messageBoxButtons, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(_Owner, text.Translate(args), caption.Translate(), messageBoxButtons);

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(_Owner, messageResult.Message.Translate(args), caption.Translate(), messageBoxButtons);
        ...
        DialogResult IMessageBox.Show(IWin32Window _Owner, string text, string caption, MessageBoxButtons messageBoxButtons, MessageBoxIcon messageBoxIcon, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(_Owner, text.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon);

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(_Owner, messageResult.Message.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon);
        ...
        DialogResult IMessageBox.Show(IWin32Window _Owner, string text, string caption, MessageBoxButtons messageBoxButtons, MessageBoxIcon messageBoxIcon, MessageBoxDefaultButton messageBoxDefaultButton, params string[] args)
        {
            ...
            try
            {
                ...
                if (messageResult == null || !messageResult.Result)
                    return XtraMessageBox.Show(_Owner, text.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon, messageBoxDefaultButton);

                if (text.Contains("^"))
                    args = text.Substring(text.IndexOf('^') + 1).Split('^');

                if (!messageResult.Result)
                    return XtraMessageBox.Show(_Owner, messageResult.Message.Translate(args), caption.Translate(), messageBoxButtons, messageBoxIcon, messageBoxDefaultButton);
        ...
        {
            return XtraMessageBox.Show(_Owner, exception.Message, exception.GetType().Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        DialogResult IMessageBox.Show(IWin32Window _Owner, Exception exception)
        {
            if (exception.InnerException != null)
                return XtraMessageBox.Show(_Owner, exception.InnerException.Message, exception.InnerException.GetType().Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
            else
                return XtraMessageBox.Show(_Owner, exception.Message, exception.GetType().Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
